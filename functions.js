...
function bookCountsByAuthor(authors) {
  return authors.map( (auth) => ({ author: auth.name, bookCount: auth.books.length}))
}
...
function titlesByAuthorName(authorName, authors, books) {
  return books.filter( book => {
    return book.authors.filter( auth => {
      return auth.name.toLowerCase() == authorName.toLowerCase();
    }).length != 0
  }).map( (book) => (book.title))
}
...
function mostProlificAuthor(authors) {
  return authors.reduce( (prev, current) => {
    if (current.books.length > prev.books.length ){
      return current
    }else{
      return prev
    }
...
function relatedBooks(bookId, authors, books) {
  ...
  const targetAuths = books.find( (book) => book.id == bookId ).authors.map( auth => auth.name)

  return books.filter( book => {
    const intersection = book.authors.map( auth => auth.name ).filter(element => targetAuths.includes(element));
